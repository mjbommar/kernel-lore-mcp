...
fn main() {
    ...
    eprintln!("shard: {} | data_dir: {} | limit: {limit}", shard_path.display(), data_dir.display());
    ...
    for info in walk {
        ...
        let Some(entry) = tree.find_entry("m") else { continue; };
        ...
        if msg.is_none() { parse_failures += 1; }
    ...
    eprintln!("\n=== {count} messages, {total_ms}ms total ({:.2} ms/msg) ===", total_ms as f64 / count as f64);
    eprintln!("  git (walk+tree+blob): {:>8.1}ms  ({:.3} ms/msg)  {:>5.1}%", ms(t_git), ms(t_git)/count as f64, ms(t_git)/total_ms as f64*100.0);
    eprintln!("  mail_parser parse:    {:>8.1}ms  ({:.3} ms/msg)  {:>5.1}%", ms(t_parse), ms(t_parse)/count as f64, ms(t_parse)/total_ms as f64*100.0);
    eprintln!("  zstd+store append:    {:>8.1}ms  ({:.3} ms/msg)  {:>5.1}%", ms(t_store), ms(t_store)/count as f64, ms(t_store)/total_ms as f64*100.0);
    eprintln!("  data: {} MB | parse failures: {}", total_bytes/1024/1024, parse_failures);
}
