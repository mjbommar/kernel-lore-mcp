"""kernel-lore-reindex — Python wrapper around the Rust binary."""
...
_BIN_NAME = "kernel-lore-reindex"
_DEV_TREE_SENTINELS = ("Cargo.toml", "pyproject.toml", "src/bin/reindex.rs")
...
def _find_rust_binary() -> str:
    ...
    override = os.environ.get("KLMCP_REINDEX_BINARY")
    if override:
        if not _is_executable_file(override):
            raise SystemExit(f"KLMCP_REINDEX_BINARY={override} is not an executable file")
        if _same_path(override, current_wrapper):
            raise SystemExit(
                "KLMCP_REINDEX_BINARY points at the Python console-script wrapper, "
                "not the Rust binary. Point it at the built binary instead "
                "(e.g. target/release/kernel-lore-reindex)."
            )
        return override
    ...
    raise SystemExit(
        "kernel-lore-reindex binary not found on PATH. "
        "If you're in a source checkout, build it with "
        "`cargo build --release --bin kernel-lore-reindex`. "
        "Otherwise install via `cargo install --path .`, or point "
        "KLMCP_REINDEX_BINARY at the built binary "
        "(e.g. target/release/kernel-lore-reindex)."
    )
